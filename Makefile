...
lint: ## check style with flake8
	flake8 aiorest_client tests

test t: $(VIRTUAL_ENV)/bin/py.test ## run tests quickly with the default Python
	py.test
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source aiorest_client -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/aiorest_client.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ aiorest_client
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
